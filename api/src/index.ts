...
async function runRotation() {
  try {
    ...
    for (const proxy of proxiesToRotate) {
      try {
        logger.info({ proxyId: proxy.id }, "Rotating proxy");
        ...
        const containerId = await createAndStartContainer({
          ...
          nordUser, nordPass, country: proxy.country, city: proxy.city,
        ...
        await db.update(shProxiesTable)
          .set({ containerId, status: "starting", rotationNextAt: nextAt })
          .where(eq(shProxiesTable.id, proxy.id));
        logger.info({ proxyId: proxy.id }, "Proxy rotated successfully");
      } catch (err) {
